    ...
    for epoch in range(trainer.start_epoch, cfg.end_epoch):
        ...
        for itr, (inputs, targets, meta_info) in enumerate(trainer.batch_generator):
            ...
            if itr % 20 == 0:
                ...
                # dump the losses
                if cfg.predict_type == 'angles':
                    writer.add_scalar('pose loss', loss['pose'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('shape_reg loss', loss['shape_reg'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('vertex loss', loss['vertex_loss'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('shape', loss['shape_loss'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('joints loss', loss['joints_loss'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('joints2d_loss loss', loss['joints2d_loss'].item(),
                                      epoch * len(trainer.batch_generator) + itr)

                elif cfg.predict_type == 'vectors':
                    if cfg.predict_2p5d:
                        writer.add_scalar('joint_2p5d_hm loss', loss['joint_2p5d_hm'].item(),
                                          epoch * len(trainer.batch_generator) + itr)
                    else:
                        writer.add_scalar('joint_vec loss', loss['joint_vec'].item(), epoch * len(trainer.batch_generator) + itr)
                        writer.add_scalar('joints loss', loss['joints_loss'].item(), epoch * len(trainer.batch_generator) + itr)
                        writer.add_scalar('joints2d_loss loss', loss['joints2d_loss'].item(),
                                          epoch * len(trainer.batch_generator) + itr)

                if cfg.hand_type == 'both':
                    writer.add_scalar('rel_trans loss', loss['rel_trans'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('hand_type loss', loss['hand_type'].item(), epoch * len(trainer.batch_generator) + itr)
                ...
                writer.add_scalar('cls loss', loss['cls'].item(), epoch * len(trainer.batch_generator) + itr)

                writer.add_scalar('heatmap loss', loss['joint_heatmap'].item(), epoch * len(trainer.batch_generator) + itr)

                writer.add_scalar('Learning rate', trainer.lr_scheduler.get_last_lr()[-1], epoch * len(trainer.batch_generator) + itr)
                writer.add_scalar('total loss', sum(loss[k].item() for k in loss), epoch * len(trainer.batch_generator) + itr)

                if cfg.use_2D_loss and ((not cfg.predict_2p5d) or (cfg.predict_type=='angles')) :
                    writer.add_scalar('cam trans', loss['cam_trans'].item(), epoch * len(trainer.batch_generator) + itr)
                    writer.add_scalar('cam scale', loss['cam_scale'].item(), epoch * len(trainer.batch_generator) + itr)
